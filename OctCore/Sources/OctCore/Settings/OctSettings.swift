...
public struct OctSettings: Codable, Equatable, Sendable {
	public static let defaultPasteLastTranscriptHotkey = HotKey(key: .v, modifiers: [.option, .shift])
	public static let baseSoundEffectsVolume: Double = OctCoreConstants.baseSoundEffectsVolume
	...
	public init(
		...
		soundEffectsVolume: Double = OctSettings.baseSoundEffectsVolume,
		...
		minimumKeyTime: Double = OctCoreConstants.defaultMinimumKeyTime,
		...
		pasteLastTranscriptHotkey: HotKey? = OctSettings.defaultPasteLastTranscriptHotkey,
	...
	public init(from decoder: Decoder) throws {
		...
		for field in OctSettingsSchema.fields {
...
private struct SettingsField<Value: Codable & Sendable> {
	...
	let keyPath: WritableKeyPath<OctSettings, Value>
	...
	init(
		_ key: HexSettingKey,
		keyPath: WritableKeyPath<OctSettings, Value>,
...
private struct AnySettingsField {
	let key: HexSettingKey
	let decode: (KeyedDecodingContainer<HexSettingKey>, inout OctSettings) throws -> Void
	let encode: (OctSettings, inout KeyedEncodingContainer<HexSettingKey>) throws -> Void

	func decode(into settings: inout OctSettings, from container: KeyedDecodingContainer<HexSettingKey>) throws {
		try decode(container, &settings)
	}

	func encode(_ settings: OctSettings, into container: inout KeyedEncodingContainer<HexSettingKey>) throws {
		try encode(settings, &container)
	}
}

private enum OctSettingsSchema {
	static let defaults = OctSettings()
