...
private let logger = OctLog.keyEvent
...
class KeyEventMonitorClientLive {
  ...
  private let queue = DispatchQueue(label: "com.sasha.oct.KeyEventMonitor", attributes: .concurrent)
  ...
  private var isPedalDown = false
  private var hasPromptedForAccessibilityTrust = false

  // Foot pedal detection
  // iKKEGOL FS2020U1IR reports keyboardType=40 (vs 92 for built-in keyboard)
  private static let pedalKeyboardType: Int64 = 40
  private static let pedalKeyCodes: Set<Int64> = [0, 11, 8] // a, b, c
    ...
    guard
      let eventTap = CGEvent.tapCreate(
        ...
        callback: { _, type, cgEvent, userInfo in
          ...
          // Foot pedal detection: intercept pedal events by keyboardType
          if (type == .keyDown || type == .keyUp),
             cgEvent.getIntegerValueField(.keyboardEventKeyboardType) == KeyEventMonitorClientLive.pedalKeyboardType,
             KeyEventMonitorClientLive.pedalKeyCodes.contains(cgEvent.getIntegerValueField(.keyboardEventKeycode)) {
            if type == .keyDown && !hotKeyClientLive.isPedalDown {
              hotKeyClientLive.isPedalDown = true
              logger.info("Foot pedal down")
              DistributedNotificationCenter.default().postNotificationName(
                Notification.Name("com.footswitch.pedalDown"), object: nil, userInfo: nil, deliverImmediately: true)
            } else if type == .keyUp && hotKeyClientLive.isPedalDown {
              hotKeyClientLive.isPedalDown = false
              logger.info("Foot pedal up")
              DistributedNotificationCenter.default().postNotificationName(
                Notification.Name("com.footswitch.pedalUp"), object: nil, userInfo: nil, deliverImmediately: true)
            }
            return nil // suppress the pedal's a/b/c keypress
          }
